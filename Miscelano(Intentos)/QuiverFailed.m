...
% Add quiver plot
quiver(X, Y, u_x * ones(size(X)), v_y * ones(size(Y)), 'k', 'AutoScaleFactor', 1.5); % Adjust AutoScaleFactor for larger arrows
xlim([0 L_x]); % Set x-axis limit
ylim([0 L_y]); % Set y-axis limit
...
for t = 1:t_nodes
    ...
    % Update quiver plot
    quiver(X, Y, u_x * ones(size(X)), v_y * ones(size(Y)), 'k', 'AutoScaleFactor', 1); % Adjust AutoScaleFactor for larger arrows
